...
CSS __``var()``__ 함수는 [사용자 지정 속성(en-US)](https://developer.mozilla.org/en-US/docs/Web/CSS/--*) , 또는 "CSS 변수" 의 값을 다른 속성의 값으로 지정할 때 사용합니다
~~~css
...
~~~
> __참고__: 대체값 구문은 사용자 지정 속성 구문과 동일하게 쉼표를 허용합니다.   그러므로 var(--foo, red, blue)의 대체값은 쉼표까지 포함한 red, blue입니다.   말하자면 첫 번째 쉼표의 뒤쪽은 모두 대체값이 되는 것입니다.  
...
/* 컴포넌트의 스타일에서: */
.component .header {
  color: var(--header-color, blue); /* header-color가 설정되지 않았으므로 파란색으로 유지되며 대체 값 */
}
...
[내용출처 MDN var()](https://developer.mozilla.org/ko/docs/Web/CSS/var())