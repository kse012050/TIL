...
### git tag [문자열] [해쉬코드]
해당되는 [해쉬코드]에 [문자열] 태그를 추가한다

### git tag [문자열(버전)] [해쉬코드] -a -m "[버전의 설명]"
git tag (버전) 와 버전에 대한 설명을 적을 수 있다
> -a -m  -> ``-am`` 으로 작성 가능

### git tag -l "[문자열]"
git tag list 중에 특정한 문자열이 들어 있는 것만 확인 가능하다
> git tag -l "v1.0.*"   -> v.1.0 의 버전들을 모두 확인할 수 있다
